...
public class ShopAction {
    ...
    public ArrayList loadData() throws IOException, FileNotFoundException  {
        ...
        if ( inputFile.exists() && inputFile.isFile() ) {
            ...
            while (fileScanner.hasNextLine()) {
                ...
                if (!Line.isEmpty()) {
                    ...
                    initData.add(new ShopData(stockCode, Title, Description, StockPrice,StockPricePence, Quantity ));
                }
            }
...
//        }
    ...
    public ArrayList displayItems(ArrayList<ShopData> storedData, String action) {
        if (action.equals("view")){
            ...
            for (int a = 0; a < storedData.size(); a++) {
                ShopData dataOut = storedData.get(a);
    ...
    public ArrayList modifyStock (ArrayList<ShopData> storedData, String action, int index) {
        if (action.equals("buy")) {
            if(storedData.get(index).getStockQuantity() > 0) {
                int newQuantity = storedData.get(index).getStockQuantity() - 1;
                storedData.set(index, new ShopData(storedData.get(index).getProductCode(),storedData.get(index).getProductTitle(),
...
//}
    ...
    public static void savedData( ArrayList<ShopData> storedData) throws IOException, FileNotFoundException {
